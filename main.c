...
static TM_ETHERNET_SSI_t SSI_Tags[] = {
	...
	"ipserver",   /* Tag 16 = link status */
	"port", /* Tag 17 = duplex status */
...
char						Carcheck[6]			={0x7F,0x40,0x01,0x07,0x0D,0x12};				// code hex Test dau nuot
char 						IDCAR1[7]				={0x00,0x00,0x00,0x00,0x00,0x00,0x00};	// luu gia tri IDCAR tu dau doc da nang
char 						UID1[30];
char 						UID1_save[30];
unsigned char 	Card_test				=0;
/*Khoi Action*/
...
//	if(cnt<48)cnt++;
//	else cnt =0;
		/*For TCR 651 nuot nhe*/
	if(c==0x7F) cnt =0;
	BufferCom3[cnt]=c;
	if(cnt==5){
		if((c==0x23)||(c==0x86)||(c==0x9D)){
			flag_TCR=0;
		}
		else flag_TCR =1;
	}
...
void 					TM_USART6_ReceiveHandler(uint8_t c) {	// RFID1
	...
	if((BufferCom1[3]==0x00)&&(cnt==BufferCom1[3]+1)&&cnt)
	{
		...
		if((BufferCom1[3]==0x01)&&(cnt==BufferCom1[3]+1)&&cnt)
	{
		Card_test=1;
	}
	if(cnt<48)cnt++;
...
uint8_t 			TM_ETHERNETSERVER_ClientConnectedCallback(struct tcp_pcb *pcb) {
	...
	if (pcb->remote_ip.addr == ip.addr) {
		/* Print to user */
		sprintf(str,"User with bad IP was trying to access to website\n");
		printf_d(str);
		/* Disable access, show error page */
		return 0;
	}
	/* Print to user */
	sprintf(str,"Connection allowed\n");
	printf_d(str);
...
void 					TM_ETHERNETSERVER_ClientDisconnectedCallback(void) {
	/* Print to user */
	sprintf(str,"Client disconnected\n");
	printf_d(str);
}
...
uint16_t TM_ETHERNETSERVER_SSICallback(int iIndex, char *pcInsert, int iInsertLen) {
	...
	if (iIndex < 4) {
		...
		switch (iIndex) {
			...
			case 2:
				...
				//status = TM_DISCO_LedIsOn(LED_RED);
					{
						sprintf(str,"TM_DISCO_LedIsOn(LED_RED)");
						printf_d(str);
					}
				break;
			case 3:
		...
		/* #hardware */
		strcpy(pcInsert, "STM32F407VE-TIS");
	} else if (iIndex == 19) {
		/* #rtc_time */
...
void 					vTimerCallback4( xTimerHandle  pxTimer ){	// Read wiegang 2. 10ms
     ...
     lExpireCounters[ lArrayIndex ] += 1;	
			...
			if(crc_26bit(count_2,card_2)){
			 ...
			 sprintf(str,"w2,%03u%05u",(uint32_t)card_2>>16,(uint32_t)card_2&0xFFFF);
			 countTimeOut2 = 0;
			 card_2 = 0;
			 count_2 = 0;
			 flag_finish_2 = 1;				
